...
public class Sequence implements Iterable<Integer> {
  ...
  public Sequence(int[] command) {
    ...
    switch (sequenceType) {
      ...
      case GEOMETRIC: // call the constructor of the GeometricSequenceGenerator class to create
        // an iterator over a geometric sequence
      ...
      case DIGIT_PRODUCT: // call the constructor of the DigitProductSequenceGenerator class to
        // create an iterator over a geometric sequence
   ...
   * @return true if the syntax of userCommand is correct with respect to the program specification,
   *         false otherwise
   */
  private static boolean checkCommandSyntax(String[] userCommand) {
    // add the time complexity of this method, where the problem size N is the length
    // of the userCommand array provided as input argument parameter
    // Time Complexity: O(1)
    boolean syntaxError = false;
    switch (userCommand[0].trim()) {
      case "0": // Arithmetric progression
  ...
  /**
   * Override the method in the interface Iterator
   */
